...
function sanitize_cgroups() {
  # PATCH: Detect cgroup v2 and skip v1-specific setup
  # Check if we're running under cgroup v2 (unified hierarchy)
  # In cgroup v2, all controllers show hierarchy 0 in /proc/cgroups
  # and /proc/self/cgroup has a single line starting with "0::"
  if grep -q "^0::" /proc/self/cgroup 2>/dev/null; then
    echo "Detected cgroup v2 (unified hierarchy) - skipping v1-specific cgroup setup"
    echo "Docker will handle cgroup v2 natively"
    return 0
  fi
  
  # Original cgroup v1 setup logic follows...
  echo "Detected cgroup v1 - performing standard cgroup setup"
...
EOF
  ...
  # PATCH: Comment out trap to keep Docker running after script exits
  # The pipeline needs Docker to stay running for subsequent BOSH commands
...
function main() {
  # PATCH: Fix multi-IP issue by selecting only the first non-loopback IPv4 address
  # Original code could return multiple IPs (e.g., container IP + docker0 bridge IP)
